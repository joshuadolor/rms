<template>
  <div class="space-y-4" data-testid="contact-and-links-section">
    <h4 class="font-medium text-charcoal dark:text-white flex items-center gap-2">
      <span class="material-icons text-slate-500 dark:text-slate-400 text-lg">contact_phone</span>
      Contact &amp; links
    </h4>
    <p class="text-xs text-slate-500 dark:text-slate-400">
      Add phone numbers and social or website links. Only active entries appear on your public page.
    </p>

    <!-- Add / Edit form -->
    <div
      v-if="showForm"
      class="rounded-xl bg-slate-50 dark:bg-zinc-800/50 border border-slate-200 dark:border-slate-800 p-4 sm:p-5 space-y-4"
      data-testid="contact-form-card"
    >
      <h5 class="text-base font-semibold text-charcoal dark:text-white">
        {{ editingContact ? 'Edit contact' : 'Add contact or link' }}
      </h5>
      <form
        novalidate
        class="space-y-4"
      ...
      >
        ...
        <div>
          <label for="contact-value" class="block text-sm font-semibold text-charcoal dark:text-white mb-1">
            {{ isLinkType(form.type) ? 'URL' : 'Phone number' }} <span class="text-red-500">*</span>
          </label>
          <input
            id="contact-value"
            v-model="form.value"
            data-testid="contact-value-input"
            :type="isLinkType(form.type) ? 'url' : 'tel'"
            :autocomplete="isLinkType(form.type) ? 'url' : 'tel'"
            maxlength="500"
            :placeholder="isLinkType(form.type) ? 'https://...' : 'e.g. +1 234 567 8900'"
            class="w-full rounded-lg ring-1 ring-slate-200 dark:ring-zinc-700 bg-white dark:bg-zinc-800 border-0 py-3 px-4 text-charcoal dark:text-white focus:ring-2 focus:ring-primary min-h-[44px]"
        ...
        <div class="flex flex-wrap gap-2 pt-2">
          <AppButton type="submit" variant="primary" class="min-h-[44px]" :disabled="saving" data-testid="contact-form-submit">
            ...
            {{ saving ? 'Saving…' : (editingContact ? 'Save' : 'Add') }}
          </AppButton>
    ...
    <div v-else>
      <AppButton variant="secondary" size="sm" class="min-h-[44px]" data-testid="contact-add-button" @click="openAddForm">
        <template #icon>
    ...
    <div v-if="loading" class="space-y-3">
      <div v-for="i in 2" :key="i" class="h-16 rounded-xl bg-slate-100 dark:bg-zinc-800 animate-pulse" />
    </div>

    <div v-else-if="errorList" class="rounded-xl bg-slate-50 dark:bg-zinc-800/50 border border-slate-200 dark:border-slate-800 p-4 text-center">
      <p class="text-slate-500 dark:text-slate-400 text-sm">{{ errorList }}</p>
      <AppButton variant="secondary" size="sm" class="mt-3 min-h-[44px]" @click="fetchContacts">Retry</AppButton>
    </div>

    <div v-else-if="!contacts.length" class="rounded-xl bg-slate-50 dark:bg-zinc-800/50 border border-slate-200 dark:border-slate-800 p-6 text-center">
      <span class="material-icons text-4xl text-slate-300 dark:text-slate-600">contact_phone</span>
      <p class="mt-3 text-sm text-slate-500 dark:text-slate-400">No contacts or links yet. Add one so customers can reach you.</p>
    </div>
    ...
    <ul v-else class="space-y-3" data-testid="contacts-list">
      <li
        v-for="c in contacts"
        :key="c.uuid"
        class="rounded-xl bg-slate-50 dark:bg-zinc-800/50 border border-slate-200 dark:border-slate-800 p-4 sm:p-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"
        data-testid="contact-item"
      >
        <div class="min-w-0 flex-1">
          <div class="flex flex-wrap items-center gap-2">
            <span class="font-semibold text-charcoal dark:text-white text-sm">{{ c.typeLabel }}</span>
            <span
              v-if="!c.is_active"
              class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-200 dark:bg-zinc-700 text-slate-600 dark:text-slate-400"
            >
              Hidden
            </span>
          </div>
          <p class="text-slate-600 dark:text-slate-300 text-sm font-mono mt-0.5 break-all">{{ displayValue(c) }}</p>
          <p v-if="c.label" class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{{ c.label }}</p>
        </div>
...
const props = defineProps({
  ...
  /** When true, fetch contacts on mount and when restaurantUuid changes. */
...
function displayValue(c) {
  ...
  if (isLinkType(c.type) && v.length > 50) return v.slice(0, 47) + '…'
  return v
}
...
function validateForm() {
  ...
  if (isLinkType(type)) {
    if (!val) err.value = 'URL is required.'
    else if (!URL_RE.test(val)) err.value = 'Please enter a valid URL.'
    else if (val.length > 500) err.value = 'URL must be at most 500 characters.'
  } else {
    if (!val) err.value = 'Phone number is required.'
    else if (val.length > 500) err.value = 'Phone number must be at most 500 characters.'
  }
...
watch(
  () => [props.restaurantUuid, props.active],
  ([uuid, active]) => {
